...
export default class Map extends Component {
  ...
  async componentDidMount() {
    // Checks to see if user has visited this page before using localStorage
    ...
    fetch(`${config.API_ENDPOINT}/artists`, {
      ...
      .then((res) => {
        ...
        res.json().then((artistData) => {
          ...
          // TODO LATER: Make this a reusable function.
  ...
  forwardGeocoder = (query) => {
    ...
    for (let i = 0; i < customData.features.length; i++) {
      ...
      if (
        feature.properties.artist_name
          .toLowerCase()
      ...
      <>
        <div className="sidebarStyle" id="DEBUGGER_REMOVE_BEFORE_DEPLOY">
          <div>
            ...
            <p id="help" onClick={() => this.togglePopUp()}>
              HELP
            </p>
          </div>
        </div>
        ...
        {this.state.tutorial && (
          <PopUp closeWindow={this.togglePopUp} version={config.VERSION} />
        )}
      </>
    );
  }
