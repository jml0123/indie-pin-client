  ...
  handleArtistLookup(e) {
    if (!e.target.value.includes("https://open.spotify.com")) {
      ...
      this.setState({
        ...this.state,
        validator:
          "Must be a valid spotify artist link (e.g. https://open.spotify.com/artist/:artist_id)",
      });
      return;
    }
  ...
  render() {
      ...
      <div className="artist-form-header">
        <div className="artist-img-wrapper--form">
          <img
            ...
            alt={this.state.artist.name + " spotify profile img"}
          />
        </div>
    ...
    const newArtistForm = (
      <>
        ...
        <label htmlFor="spotify-artist-link">
          Pin an artist using their Spotify URL
        </label>
        <input
          type="text"
          id="spotify-artist-link"
          onChange={(e) => this.handleArtistLookup(e)}
        ></input>
        <button disabled={!this.state.artist}>Submit</button>
        {this.state.validator && (
          <div className="result-banner">{this.state.validator}</div>
        )}
        {this.state.result && (
          <div className="result-banner">{this.state.result}</div>
        )}
      </>
    );

    const form = !this.props.auth ? null : this.props.auth.token ? (
      newArtistForm
    ) : (
      <>
        <i className="fa fa-map-pin" aria-hidden="true" />
    ...
    return (
      <div className="new-artist-form">
        <form onSubmit={this.handleSubmit}>
          {artistLevel}
          {artistInfo}
          <div className="form-input-container">{form}</div>
        </form>
      </div>
    );
  }
